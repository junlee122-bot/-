...
기준선이 달라 직접 비교가 어렵습니다. 그래서 **Pinnacle 1X2 공정확률 → 포아송
득점 모델(λ 역산) → 파생 마켓 공정확률**을 계산해 edge/EV를 평가합니다(축구·하키
3갈래 종목만). 이는 **모델 추정값**이라 1X2보다 신뢰도를 낮춰(×0.6) 반영하며,
대시보드/로그에 `model_based` 로 표시됩니다.
